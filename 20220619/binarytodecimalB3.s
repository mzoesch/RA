...
nextCh: # ==============================
    ...
    sub     $a1, $a1, 1
    ble     $a1, 0, strEnd      # Schleifen exit
    
    lb      $t2, ($a0)          # e. g.: 0x10010020
    andi    $t2, $t2, 0x0F      # Von ASCII nach HEX
    
    bnez    $t2, eins           # j zu 'eins' falls $t2 /= 0
                                # Geht automatisch zu 'null' falls obiges nicht wahr ist
...
eins:				# Bit war 1
   	...
   	lw	$t3, ($t1)	# Lade Wert 2^7, 2^6, etc. in $t3
   	add	$t7, $t7, $t3	# Addiere zu den anderen 'einsen'
